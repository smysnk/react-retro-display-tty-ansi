...
const maybeIt = process.env.ANSI_GALLERY_PARITY === "1" ? it : it.skip;
...
describe("real tty byte parity ansi gallery corpus", () => {
  it("tracks the full ansi gallery manifest locally", async () => {
  ...
  maybeIt("replays every ansi-gallery asset through the sampled parity runner", async () => {
    const entries =
      galleryIds.length > 0
